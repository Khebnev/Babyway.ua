...
function virtuemartBuildRoute(&$query) {
	...
	if (strrpos($_SERVER["REQUEST_URI"], "component/virtuemart") !== false) {
		if (isset($query['task'])) {
			$segments[] = $helper->lang($query['task']);
			unset($query['task']);
		}
		...
		if (strrpos($_SERVER["REQUEST_URI"], "2-uncategorised") !== false) {
		if (isset($query['task'])) {
			$segments[] = $helper->lang($query['task']);
			unset($query['task']);
		}
	...
	if (strrpos($_SERVER["REQUEST_URI"], "by") !== false) {
		if (isset($query['task'])) {
			$segments[] = $helper->lang($query['task']);
			unset($query['task']);
		}
	...
	if (strrpos($_SERVER["REQUEST_URI"], "keyword") !== false) {
		if (isset($query['task'])) {
			$segments[] = $helper->lang($query['task']);
			unset($query['task']);
		}
	...
	$category_url = substr($_SERVER["REQUEST_URI"], 1, strlen($_SERVER["REQUEST_URI"]));
	$category_url = explode("?", $category_url)[0];
	$url = explode("?", $_SERVER["REQUEST_URI"])[0];
	$url = explode("/", $url);
	$product_url = explode("?", $_SERVER["REQUEST_URI"])[0];
	$product_url = explode("/", $product_url);
	$product_url = $product_url[count($product_url) - 1];
	$product_url = str_replace("-detail", "", $product_url);
	...
	if (
		...
		isset($_GET["virtuemart_product_id"]) &&
		intval($_GET["virtuemart_product_id"]) !== false &&
		strrpos($_SERVER["REQUEST_URI"], "index.php?") !== false
	   ) {
	   	$productId = intval($_GET["virtuemart_product_id"]);
		$q = "SELECT * FROM #__virtuemart_products_ru_ru WHERE virtuemart_product_id = \"$productId\"";
		$db->setQuery($q);
	...
	$q = "SELECT * FROM #__menu WHERE path = \"$category_url\"";
	$db->setQuery($q);
	$result = $db->loadObject();
	...
	if (!empty($result)) {
		...
		$q = "SELECT * FROM #__virtuemart_products_ru_ru WHERE slug = \"$product_url\"";
		$db->setQuery($q);
		$result = $db->loadObject();
		...
		if (!empty($result)) {
			...
			$q = "SELECT * FROM #__menu WHERE path = \"$url\"";
			$db->setQuery($q);
			$result = $db->loadObject();
